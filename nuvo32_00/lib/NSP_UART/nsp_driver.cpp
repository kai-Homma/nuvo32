...
{		
	Serial2.begin(38400); // IO16 Rx, IO17 Tx UART 2 115200����ͨ�Ų�������
}
...
UINT16 N_UART_READ(UINT16 READ_SIZE)
{
    ...
    UINT32 u32Count;
	Serial2.readBytes(u8ReadBuffer, READ_SIZE);
    return u32Count;	
}
...
UINT16 N_UART_WRITE(UINT16 WRITE_SIZE)
{
    ...
    UINT32 u32Count;
	Serial2.write(u8WriteBuffer, WRITE_SIZE);
    return u32Count;
}
//----------------------------------
...
//----------------------------------
UINT8 N_GET_ISP(UINT16* CHECKSUM_BYTES)
{
	UINT8 RTN = 0;
...
UINT8 N_ISP_WRITE_END(void)
{
	...
	RTN = CHECK_UART_READ_RIGHT();
	Serial.println(RTN);
	Serial.println(u8WriteBuffer[0]);
	HOST_CMD_INTERVAL();
	while ((N_READ_STATUS(&u8NSP_STATUS1) != 1) || (u8NSP_STATUS1 != CMD_VALID))
	{
		HOST_CMD_INTERVAL();
...
UINT8 N_AUTO_SLEEP_DIS()
{
	...
	Serial.println(u8RX_ERROR_COUNT);
	N_UART_WRITE(u16CMD_TX_BYTE);
	HOST_UART_WRITE_DURATION();
	N_UART_READ(u16CMD_RX_BYTE);
	HOST_UART_READ_DURATION();
